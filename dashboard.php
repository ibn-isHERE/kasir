...
if (isAdmin()) {
    ...
    $totalPelanggan = mysqli_fetch_assoc(query($conn, "SELECT COUNT(*) as total FROM pelanggan"))['total'];
    $totalPetugas = mysqli_fetch_assoc(query($conn, "SELECT COUNT(*) as total FROM user WHERE Role = 'petugas'"))['total'];
    
    $penjualanHariIni = mysqli_fetch_assoc(query($conn, "SELECT COUNT(*) as total, COALESCE(SUM(TotalHarga), 0) as omset FROM penjualan WHERE DATE(TanggalPenjualan) = CURDATE()"));
    ...
    // Statistik Petugas
    $userId = $userInfo['id'];
    $transaksiHariIni = mysqli_fetch_assoc(query($conn, "SELECT COUNT(*) as total, COALESCE(SUM(TotalHarga), 0) as omset FROM penjualan WHERE UserID = $userId AND DATE(TanggalPenjualan) = CURDATE()"));
...
<?php if (isAdmin()): ?>
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(250px, 1fr)); gap: 20px; margin-bottom: 30px;">
        ...
        <div class="card" style="background: linear-gradient(135deg, #00b894 0%, #00cec9 100%); color: white; border: none;">
            <div class="card-body">
                <div style="display: flex; justify-content: space-between; align-items: start;">
                    <div>
        ...
        <div class="card" style="background: linear-gradient(135deg, #fdcb6e 0%, #e17055 100%); color: white; border: none;">
            <div class="card-body">
                <div style="display: flex; justify-content: space-between; align-items: start;">
                    <div>
                        <div style="font-size: 0.9em; opacity: 0.9; margin-bottom: 5px;">Total Pelanggan</div>
                        <div style="font-size: 2.5em; font-weight: 700; font-family: 'Crimson Pro', serif;"><?php echo $totalPelanggan; ?></div>
                    </div>
                    <div style="font-size: 3em; opacity: 0.3;">👥</div>
                </div>
            </div>
        ...
        <div class="card" style="background: linear-gradient(135deg, #3742fa 0%, #5352ed 100%); color: white; border: none;">
            <div class="card-body">
                <div style="display: flex; justify-content: space-between; align-items: start;">
                    <div>
                            ...
                            <tbody>
                                <?php while ($row = fetchArray($stokRendah)): ?>
                                    <tr>
                                        ...
                                        <td>
                                            <a href="produk.php?edit=<?php echo $row['ProdukID']; ?>" class="btn btn-warning btn-sm">
                                                Update Stok
                                            </a>
                                        </td>
                                    </tr>
...
<?php else: ?>
    ...
    <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); gap: 15px; margin-bottom: 30px;">
        <a href="pos.php" class="btn btn-primary" style="padding: 20px; justify-content: center; font-size: 1.1em;">
            💳 Transaksi Baru
        </a>
        <a href="stok.php" class="btn btn-info" style="padding: 20px; justify-content: center; font-size: 1.1em;">
            📦 Cek Stok
        </a>
        <a href="laporan_petugas.php" class="btn btn-secondary" style="padding: 20px; justify-content: center; font-size: 1.1em;">
            📄 Laporan
        </a>
    </div>
        ...
        <div class="card-body">
                        ...
                        <tbody>
                            <?php while ($row = fetchArray($transaksiTerbaru)): ?>
                                <tr>
                                    ...
                                    <td><?php echo htmlspecialchars($row['NamaPelanggan']); ?></td>
                                    <td><strong>Rp <?php echo number_format($row['TotalHarga'], 0, ',', '.'); ?></strong></td>
                                </tr>
                            <?php endwhile; ?>
                        </tbody>
                    </table>
            ...
            <?php else: ?>
                <p style="color: var(--text-light); text-align: center; padding: 40px;">
                    Belum ada transaksi hari ini. <a href="pos.php">Mulai transaksi pertama</a>
                </p>
            <?php endif; ?>
        </div>
    </div>

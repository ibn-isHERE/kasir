...
// Statistik per kasir
$kasirStats = query($conn, "SELECT u.Username, COUNT(*) as jumlah, SUM(p.TotalHarga) as omset
                             FROM penjualan p 
                             JOIN user u ON p.UserID = u.UserID
                             WHERE DATE(p.TanggalPenjualan) BETWEEN '$filterMulai' AND '$filterSampai'
                             GROUP BY u.UserID
...
<div class="card">
    ...
    <div class="card-body">
        <form method="GET" action="" style="display: grid; grid-template-columns: 1fr 1fr auto auto; gap: 15px; align-items: end;">
            ...
            <a href="cetak_laporan.php?mulai=<?php echo $filterMulai; ?>&sampai=<?php echo $filterSampai; ?>" 
